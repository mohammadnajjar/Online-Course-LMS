<?php
/**
 * Static functions used in the TUTORMATE plugin.
 *
 * @package Tutormate
 */
...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		...
		$import_file_info = apply_filters('tutormate_pre_download_import_files', $import_file_info);
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			...
			$content_filename = apply_filters( 'tutormate_downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . apply_filters( 'tutormate_downloaded_content_file_suffix_and_file_extension', '.xml' );
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$widget_filename = apply_filters( 'tutormate_downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . apply_filters( 'tutormate_downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$customizer_filename = apply_filters( 'tutormate_downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . apply_filters( 'tutormate_downloaded_customizer_file_suffix_and_file_extension', '.dat' );
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'tutormate' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'tutormate' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'tutormate' ),
					'<strong>',
					'</strong>',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'tutormate' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'tutormate' )
			);
		}
	...
	public static function get_log_path() {
		...
		$upload_path = apply_filters( 'tutormate_upload_file_path', trailingslashit( $upload_dir['path'] ) );

		$log_path = $upload_path . apply_filters( 'tutormate_log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . apply_filters( 'tutormate_log_file_suffix_and_file_extension', '.txt' );
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$filetype  = wp_check_filetype( basename( $log_path ), apply_filters( 'tutormate_file_mimes', $log_mimes ) );

		// Prepare an array of post data for the attachment.
		$attachment = array(
			'guid'           => self::get_log_url( $log_path ),
			'post_mime_type' => $filetype['type'],
			'post_title'     => apply_filters( 'tutormate_attachment_prefix', esc_html__( 'Tutormate Demo Import - ', 'tutormate' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function get_log_url( $log_path ) {
		$upload_dir = wp_upload_dir();
		$upload_url = apply_filters( 'tutormate_upload_file_url', trailingslashit( $upload_dir['url'] ) );
	...
	public static function verify_ajax_call() {
		check_ajax_referer( 'tutormate-ajax-verification', 'security' );

		// Check if user has the WP capability to import data.
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'tutormate' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$file_not_provided_error = array(
			'error' => esc_html__( 'No file provided.', 'tutormate' )
		);
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Content file was not uploaded. Error: %s', 'tutormate' ),
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', 'tutormate' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', 'tutormate' ),
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
		sprintf(
			__( 'Initial max execution time = %s', 'tutormate' ),
	 ...
	 */
	public static function set_tutormate_import_data_transient( $data ) {
		set_transient( 'tutormate_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}
}
