 ...
 * Register default WP actions for TUTORMATE plugin.
 *
 * @package Tutormate
 */
...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action( 'tutormate_before_content_import_execution', array( $this, 'before_content_import_action' ), 10, 3 );
		...
		add_action( 'tutormate_after_all_import_execution', array( $this, 'after_import_action' ), 10, 3 );

		// Special widget import cases.
		if ( apply_filters( 'tutormate_enable_custom_menu_widget_ids_fix', true ) ) {
			add_action( 'tutormate_widget_settings_array', array( $this, 'fix_custom_menu_widget_ids' ) );
		}
	}
	...
	/**
	 * Execute the action: 'tutormate_before_content_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `tutormate_import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_content_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'tutormate_before_content_import', $import_files[ $selected_index ] );
	}

	/**
	 * Execute the action: 'tutormate_before_widgets_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `tutormate_import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_widget_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'tutormate_before_widgets_import', $import_files[ $selected_index ] );
	}

	/**
	 * Execute the action: 'tutormate_after_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `tutormate_import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function after_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'tutormate_after_import', $import_files[ $selected_index ] );
	}
	 ...
	 * @param array  $selected_import The data of selected import from `tutormate_import_files` filter.
	 */
